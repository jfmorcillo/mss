...
function handleServicesModule($popup) {
    if (in_array("services", $_SESSION["modulesList"])) {
        ...
        $popup->add('<br /><p>' . _T("Restart the firewall service ?") . ' <button id="restartBtn" onclick="restartFirewall(\'' . $urlRestart . '\', \'' . $urlCheck . '\')"> '. _T("Restart") .'</button> <span id="restartInfo"></span></p>');
    }
    return $popup;
}
